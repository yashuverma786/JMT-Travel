...
import { Waves, MapPin, Calendar, Users, Star, Search, Filter, Sun } from "lucide-react"
...
export default function BeachTripsPage() {
  ...
  const fetchBeachTrips = async () => {
    try {
      const response = await fetch("/api/trips?tripType=Beach")
      if (response.ok) {
        const data = await response.json()
        setTrips(data.trips || [])
      }
    } catch (error) {
      console.error("Error fetching beach trips:", error)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-400 to-cyan-500 text-white py-20">
        <div className="container mx-auto px-4 text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl md:text-6xl font-bold">Beach Packages</h1>
          </div>
          <p className="text-xl md:text-2xl mb-8 opacity-90">Relax and unwind at the world's most beautiful beaches</p>
          <div className="max-w-md mx-auto">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
              <Input
                placeholder="Search beach destinations..."
                value={searchTerm}
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">{sortedTrips.length} Beach Packages Found</h2>
        </div>
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="p-12 text-center">
              <Waves className="h-12 w-12 mx-auto text-gray-400 mb-4" />
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No beach packages found</h3>
            ...
            {sortedTrips.map((trip) => {
              ...
              return (
                <Card key={trip._id} className="overflow-hidden hover:shadow-lg transition-shadow group">
                  <div className="relative">
                    <Image
                      src={trip.featuredImage || "/placeholder.svg?height=200&width=300&query=beach+destination"}
                      alt={trip.title}
                      width={300}
                      height={200}
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-2xl font-bold text-cyan-600">₹{trip.salePrice.toLocaleString()}</span>
                          {discount > 0 && (
