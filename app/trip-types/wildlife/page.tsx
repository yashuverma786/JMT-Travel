...
import { TreePine, MapPin, Calendar, Users, Star, Search, Filter, TelescopeIcon as Binoculars } from "lucide-react"
...
export default function WildlifeTripsPage() {
  ...
  const fetchWildlifeTrips = async () => {
    try {
      const response = await fetch("/api/trips?tripType=Wildlife")
      if (response.ok) {
        const data = await response.json()
        setTrips(data.trips || [])
      }
    } catch (error) {
      console.error("Error fetching wildlife trips:", error)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-green-600 to-emerald-500 text-white py-20">
        <div className="container mx-auto px-4 text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl md:text-6xl font-bold">Wildlife Packages</h1>
          </div>
          <p className="text-xl md:text-2xl mb-8 opacity-90">
            Experience nature's wonders and encounter amazing wildlife
          </p>
          <div className="max-w-md mx-auto">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
              <Input
                placeholder="Search wildlife destinations..."
                value={searchTerm}
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">{sortedTrips.length} Wildlife Packages Found</h2>
        </div>
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No wildlife packages found</h3>
            ...
            {sortedTrips.map((trip) => {
              ...
              return (
                <Card key={trip._id} className="overflow-hidden hover:shadow-lg transition-shadow group">
                  <div className="relative">
                    <Image
                      src={trip.featuredImage || "/placeholder.svg?height=200&width=300&query=wildlife+safari"}
                      alt={trip.title}
                      width={300}
                      height={200}
