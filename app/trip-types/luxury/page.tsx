...
import { Crown, MapPin, Calendar, Users, Star, Search, Filter, Gem } from "lucide-react"
...
export default function LuxuryTripsPage() {
  ...
  const fetchLuxuryTrips = async () => {
    try {
      const response = await fetch("/api/trips?tripType=Luxury")
      if (response.ok) {
        const data = await response.json()
        setTrips(data.trips || [])
      }
    } catch (error) {
      console.error("Error fetching luxury trips:", error)
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-amber-600 to-yellow-500 text-white py-20">
        <div className="container mx-auto px-4 text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl md:text-6xl font-bold">Luxury Packages</h1>
          </div>
          <p className="text-xl md:text-2xl mb-8 opacity-90">
            Experience the finest in luxury travel with our premium packages
          </p>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">{sortedTrips.length} Luxury Packages Found</h2>
        </div>
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No luxury packages found</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sortedTrips.map((trip) => {
              ...
              return (
                <Card
                  key={trip._id}
                  className="overflow-hidden hover:shadow-lg transition-shadow group border-2 border-amber-100"
                >
                  <div className="relative">
                    <Image
                      src={trip.featuredImage || "/placeholder.svg?height=200&width=300&query=luxury+destination"}
                      alt={trip.title}
                      width={300}
                      height={200}
                  ...
                  <CardContent className="p-4">
                      ...
                      <div className="mb-4">
                        <div className="flex flex-wrap gap-1">
                          {trip.highlights.slice(0, 2).map((highlight, index) => (
                            <Badge key={index} variant="outline" className="text-xs border-amber-200">
                              {highlight}
                            </Badge>
                          ))}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-2xl font-bold text-amber-600">₹{trip.salePrice.toLocaleString()}</span>
                          {discount > 0 && (
