...
import { Users, MapPin, Calendar, Star, Search, Filter, Baby } from "lucide-react"
...
export default function FamilyTripsPage() {
  ...
  const fetchFamilyTrips = async () => {
    try {
      const response = await fetch("/api/trips?tripType=Family")
      if (response.ok) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-blue-500 to-green-500 text-white py-20">
        <div className="container mx-auto px-4 text-center">
          <div className="flex items-center justify-center gap-3 mb-4">
            ...
            <h1 className="text-4xl md:text-6xl font-bold">Family Packages</h1>
          </div>
          <p className="text-xl md:text-2xl mb-8 opacity-90">
            Create wonderful memories with your loved ones on family-friendly adventures
          </p>
          <div className="max-w-md mx-auto">
            <div className="relative">
              <Search className="absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
              <Input
                placeholder="Search family destinations..."
                value={searchTerm}
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <Card className="mb-8">
          <CardContent className="p-6">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div>
                <label className="block text-sm font-medium mb-2">Price Range (Adult)</label>
                <Slider
                  value={priceRange}
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">{sortedTrips.length} Family Packages Found</h2>
        </div>
        ...
        {loading ? (
          ...
          <Card>
            <CardContent className="p-12 text-center">
              ...
              <h3 className="text-lg font-semibold text-gray-900 mb-2">No family packages found</h3>
            ...
            {sortedTrips.map((trip) => {
              ...
              return (
                <Card key={trip._id} className="overflow-hidden hover:shadow-lg transition-shadow group">
                  <div className="relative">
                    <Image
                      src={trip.featuredImage || "/placeholder.svg?height=200&width=300&query=family+destination"}
                      alt={trip.title}
                      width={300}
                      height={200}
                  ...
                  <CardContent className="p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-2xl font-bold text-blue-600">₹{trip.salePrice.toLocaleString()}</span>
                          {discount > 0 && (
                        ...
                        <p className="text-xs text-gray-500">
                          per adult {trip.childPrice > 0 && `• ₹${trip.childPrice.toLocaleString()} per child`}
                        </p>
                      </div>
